...
void Renderer::add_display(int width, int height, std::string const& camera, std::string const& display) {
    RenderBackend* the_new_backend( new RenderBackend( width , height, camera, display ) );
...
void Renderer::start_render_loop( SceneGraph const* scene_graph ) {
    while (true) {
        ...
        for ( auto backend( render_backend_list_.begin() ); backend != render_backend_list_.end(); ++ backend ) {
            (*backend)->render( optimizer_->get_geometry_data(), optimizer_->get_light_data(), optimizer_->get_camera_data((*backend)->get_camera_name()) );
        }
    }
}
