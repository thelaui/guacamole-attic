    ...
    do {
        ...
        if (current_core) {
            ...
            case Core::CoreType::CAMERA : {
                ...
                camera_data_.insert( std::make_pair( node.get_name(), render_camera ));
...
CameraNode* Optimizer::get_camera_data(std::string const& camera_name) const {
    auto camera = camera_data_.find(camera_name);
    if (camera != camera_data_.end()) return camera->second;
