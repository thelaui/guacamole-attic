...
SceneGraph::Iterator SceneGraph::
add_node(std::string const& path_to_parent, std::string const& node_name,
         Core* core, math::mat4 const& transform) {

    Node* searched_parent(find_node(path_to_parent));
    if (!searched_parent) {
        WARNING("A node with the name %s does not exist!",
                path_to_parent.c_str());
        return Iterator();
    }

    if (has_child(searched_parent, node_name)) {
        WARNING("A node with the name %s already exists at %s!",
                node_name.c_str(), path_to_parent.c_str());
        return Iterator();
    }
...
SceneGraph::Iterator SceneGraph::
add_node(SceneGraph::Iterator const& parent, std::string const& node_name,
         Core* core, math::mat4 const& transform) {
...
SceneGraph::Iterator SceneGraph::
add_node_recursively(std::string const& path_to_parent,
                     std::string const& node_name, Core* core,
                     math::mat4 const& transform) {

    Node* searched_parent(find_node(path_to_parent,
                                    working_node_->get_path(), true));
...
SceneGraph::Iterator SceneGraph::
remove_node(std::string const& path_to_node) {
...
SceneGraph::Iterator SceneGraph::
move_node(std::string const& path_to_node, std::string const& path_to_target) {
...
SceneGraph::Iterator SceneGraph::
copy_node(std::string const& path_to_node, std::string const& path_to_target) {
...
void SceneGraph::
set_working_node(std::string const& path_to_node) {
...
SceneGraph::Iterator SceneGraph::
get_iterator(std::string const& path_to_node, IterationType type) const {
...
SceneGraph::Iterator SceneGraph::
begin(IterationType type) const {
...
SceneGraph::Iterator SceneGraph::
operator [](std::string const& path_to_node) {
...
bool SceneGraph::
has_child(Node* parent, std::string const& child_name) const {
